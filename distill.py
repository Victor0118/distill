...
from util import read_args, make_checkpoint_incrementer
from data import Sim_ZH_Combine
...
def evaluate_score(model, ds_iter, criterion, export_eval_labels=False):
    ...
    for batch in tqdm(ds_iter):
        ...
        try:
            gts.extend(batch.logits.view(-1).tolist())
...
def main():
    args = read_args(default_config="config.json")
    set_seed(args.seed)
    try:
    ...
    dataset_cls = Sim_ZH_Combine
    training_iter, dev_iter, test_iter = dataset_cls.iters(args.dataset_path, None, None,
    ...
    model = BiRNNModel(args).to(args.device)
    ...
    increment_fn = make_checkpoint_incrementer(model, args.workspace, save_last=True,
